...
const filterEl = Array.from(document.querySelectorAll(".filters span"));
...
function addToList() {
  ...
  // create list from item
  createNewItem(value);
}

function createNewItem(listValue) {
  const item = document.createElement("li");
  item.dataset.check = "Active";

  item.innerHTML = `
            <div class="checkbox">
                <img src="./images/icon-check.svg" alt="" />
            </div>
            <p class="todo">${listValue.title}</p>
            <svg
            class="bin"
...
function itemEventHandler(todo) {
  todo.firstElementChild.addEventListener("click", () => {
    todo.classList.add("checked");
    todo.dataset.check = "Completed";
    displayItemLeft();
  });
}

function removeHandler(todo) {
  todo.children[2].addEventListener("click", () => {
    todo.remove();
    displayItemLeft();
  });
}
...
function displayItemLeft() {
  const activeTodo = Array.from(
    document.querySelectorAll(".todo-container li")
  ).filter((todo) => {
    return todo.dataset.check === "Active";
  });
  activeCount.innerHTML = `${activeTodo.length} item left`;
}
...
filterEl.forEach((el) => {
  el.addEventListener("click", () => {
    filterEl.map((filter) => {
      return filter.classList.remove("filter--active");
    });
...
function handleFilters(element) {
  const listItems = Array.from(document.querySelectorAll(".todo-container li"));

  listItems.map((item) => {
    if (item.dataset.check === element.innerHTML) {
...
clearBtn.addEventListener("click", () => {
  Array.from(todoContainer.children).forEach((item) => {
    if (item.dataset.check === "Completed") {
      item.remove();
    }
  });
});
